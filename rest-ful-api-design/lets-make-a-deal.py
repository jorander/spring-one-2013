...
r = requests.put(doors.keys()[selection], data=json.dumps({'status': 'SELECTED'}), headers={'Content-Type': 'application/json'})

doors = get_doors(doors_location)
print_current_state(game_location, doors)
selection = int(raw_input('Open a door (0, 1, or 2)... '))
requests.put(doors.keys()[selection], data=json.dumps({ 'status': 'OPEN'}), headers={'Content-Type': 'application/json'})
